...
| 能力评估维度 | 8 维 Backbone + 动态 Overlay，支持跨公司、跨岗位横向对比 |
...
### 上线后核心指标（Key Metrics to Track）

| 类别 | 指标 | 定义 | 目标 |
|------|------|------|------|
| **用户增长** | WAU（周活跃用户） | 每周至少使用 1 次的独立用户数 | 上线 3 个月：500+ |
| **用户增长** | 邀请码转化率 | 注册用户 / 使用邀请码访问的用户 | > 60% |
| **核心使用** | 面试记录频率 | 每用户每周提交的面试记录数 | ≥ 1.5 条/活跃用户/周 |
| **核心使用** | Layer 2 触发率 | 拥有 2+ 轮面试记录后触发 Role Debrief 的用户比例 | > 70% |
| **核心使用** | Layer 3 触发率 | 拥有 2+ 个岗位数据后查看 Career Growth 的用户比例 | > 40% |
| **留存** | Week 1 留存率 | 注册 7 天后仍活跃的用户比例 | > 35% |
| **留存** | Month 1 留存率 | 注册 30 天后仍活跃的用户比例 | > 20% |
| **AI 质量** | Schema 合规率 | AI 返回结果通过 JSON Schema 校验的比例 | > 95% |
| **AI 质量** | 用户编辑率 | 用户对 AI 生成的分析结果进行手动修改的比例（越低越好） | < 15% |
| **AI 质量** | Agent 错误率 | Edge Function 调用返回 5xx 的比例 | < 2% |
| **价值感知** | NPS | 净推荐值（"你是否会推荐给正在面试的朋友？"） | > 40 |
| **价值感知** | "Aha Moment" 到达率 | 完成首次面试分析并查看结果的用户比例 | > 80% |
| **商业化** | Paywall 转化率 | 免费用户升级付费的比例（如未来引入付费） | 待定 |

> **North Star Metric 建议**：**"每周被分析的面试记录数"**——它同时反映了用户活跃度（有人用）、产品价值（用户愿意持续记录）、和数据飞轮效应（更多数据 → 更好的 Career Growth 分析）。
